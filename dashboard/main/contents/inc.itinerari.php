...
if(!isset($ord)) $ord = my_statevar_read("ordItinerari");
if(!isset($searchstr)) $searchstr = my_statevar_read("searchstrItinerari");
if(!isset($nxpage)) $nxpage = my_statevar_read("nxpageItinerari");
...
if(!isset($searchTipo)) $searchTipo = my_statevar_read("searchTipo");
...
if($azione=="delItm")
{
    $sql = "DELETE FROM itinerari WHERE id=$id";
    $mydb->ExecSql($sql);
    
    $sql = "DELETE FROM itinerari_testi WHERE id_rel=$id";
    $mydb->ExecSql($sql);
    
    //CANCELLARE I FILES
    $sql = "DELETE FROM itinerari_immagini WHERE id_rel=$id";
    $mydb->ExecSql($sql);
}
?>
<script>
...
function delItm(id){
        ...
        if(confirm('ATTENZIONE: confermi la cancellazione dell\'itinerario?'))
        {
...
if(isset($searchTipo) && $searchTipo!="" )
{
    $cond .= " AND ti.info = '$searchTipo' ";
}

$sql = "SELECT COUNT(*) FROM itinerari as ti $from WHERE 1 $cond";
$mydb->DoSelect($sql);
$ntot = 0;
...
$sql = "SELECT ti.* FROM itinerari as ti  WHERE  ti.id_circuito=$id_location  $cond ORDER BY $orderby LIMIT $nn, $nxpage";
$a_ = $mydb->DoSelect($sql);
 ...
 ?>
    ...
    <div style='text-align: left'>
            <a href="?MSID=<?= $MSID ?>&tab=<?=$tab?>&sz=cms,4"><img src="main/contents/icone/add.png" class="ico24" align="absmiddle" /> Aggiungi itinerario</a>
    </div>
    ...
    <form name="frmins" method="post" action="<? echo $action ?>" >
        ...
        <div class="testo2" align="left">
            ...
            Filtra per tipologia: <select name="searchTipo" onchange="refresh()">
                <option value="">-tutte-</option>
                <option value="Itinerario" <?if($searchTipo=="Itinerario") echo "selected"?> >Itinerario</option>
                <option value="Format"  <?if($searchTipo=="Format") echo "selected"?> >Format</option>
            </select>
            <div class="spessore"></div>
   ...
   <div class="spessore" style="height:10px"></div>
        ...
        <table class="tablegrid3 tableresp" cellpadding="6" cellspacing="0">
            <thead>
                <tr>
                    <th></th>
                    <th  onclick="setOrd('nome')"><span class="underlined cliccabile">Itinerario</span></th>
                    <!--
...
my_statevar_create("searchTipo",$searchTipo);
?>
<? } elseif($isubsezione==2){ ?>
    
    <?
    if(!isset($tab2) || $tab2=="" || $tab2>2 ) $tab2=0;
    ?>
    ...
    <div class="box_cnt">
        ...
        elseif($tab2==2){
            include "itinerario_altro.inc.php";
        }
        elseif($tab2==1){
            include "itinerario_testi.inc.php";
        }
        ?>
      </div> 
