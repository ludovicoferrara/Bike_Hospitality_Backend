...
if($azione=="save3")
{
    foreach($usrf_descriz as $k => $descriz)
    {
        ...
        $sql = "UPDATE $tbl_websites_testi SET testo='$testo' WHERE  id_rel=$idws AND lang='$l'";
        $mydbNW->ExecSql($sql);
    }
...
$sql = "SELECT ta.id as idws FROM $tbl_websites as ta WHERE ta.id_anag=$id ";
$mydbNW->DoSelect($sql);
$rws = $mydbNW->GetRow();
$idws = $rws['idws'];

$sql = "SELECT ta.id as idws, tt.* FROM $tbl_websites as ta, $tbl_websites_testi as tt  WHERE ta.id_anag=$id AND ta.id=tt.id_rel";
$a_des = $mydbNW->DoSelect($sql);

if(count($a_des)==0)
{
    foreach($_vlanguage as $k => $v){
		    	$sql = "INSERT INTO $tbl_websites_testi (id_rel, lang) VALUES ($idws, '$v')";
                        $mydbNW->ExecSql($sql);
