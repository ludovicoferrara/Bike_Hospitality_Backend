...
function delItm(id){
        ...
        if(confirm('ATTENZIONE: confermi la cancellazione della stazione?'))
        {
...
switch ($ord){
	default: 
        {
            $ord = 0;
            $orderby = "ti.provincia ASC, ti.comune ASC";
        }
}
...
$sql = "SELECT COUNT(*) FROM stazioni as ti $from WHERE 1 $cond";
$mydb->DoSelect($sql);
$ntot = 0;
...
$sql = "SELECT ti.* FROM www_bikeh.stazioni as ti WHERE  ti.id_circuito=$id_location  $cond ORDER BY $orderby LIMIT $nn, $nxpage";
$a_ = $mydb->DoSelect($sql);
 ...
 ?>
    ...
    <div style='text-align: left'>
            <a href="?MSID=<?= $MSID ?>&tab=<?=$tab?>&sz=cms,4"><img src="main/contents/icone/add.png" class="ico24" align="absmiddle" /> Aggiungi stazione</a>
    </div>
...
?>
   ...
   <div class="spessore" style="height:10px"></div>
        ...
        if(is_array($a_)) foreach($a_ as $i => $r)
        {
            ...
            $indirizzo = mydecodeTxt($r['indirizzo']);
        ...
        <tr>
            ...
            <td data-label="Provincia"><?=$indirizzo?></td>
