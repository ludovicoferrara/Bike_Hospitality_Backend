...
if($azione=="save3")
{
    foreach($usrf_descriz as $k => $descriz)
    {
        ...
        $sql = "UPDATE itinerari_testi SET testo='$testo' WHERE  id_rel=$id AND lang='$l'";
        $mydbNW->ExecSql($sql);
    }
...
$sql = "SELECT ti.id , tt.* FROM itinerari as ti, itinerari_testi as tt  WHERE ti.id=$id AND ti.id=tt.id_rel";
$a_des = $mydbNW->DoSelect($sql);

if(count($a_des)==0)
{
    foreach($_vlanguage as $k => $v){
		    	$sql = "INSERT INTO itinerari_testi (id_rel, lang) VALUES ($id, '$v')";
                        $mydbNW->ExecSql($sql);
                        
    	}
        
    $sql = "SELECT ti.id , tt.* FROM itinerari as ta, itinerari_testi as tt  WHERE ti.id=$id AND ti.id=tt.id_rel";
    $a_des = $mydbNW->DoSelect($sql);
}
