...
if($azione=="save3")
{
    foreach($usrf_descriz as $k => $descriz)
    {
        ...
        $testo = addslashes($descriz);
        $testo = str_replace("<br>","\n",$testo);
        $testo = str_replace("</p>","\n",$testo);
        $testo = str_replace("<p>","",$testo);
        $testo = strip_tags($testo);
        $sql = "UPDATE guide_testi SET testo='$testo' WHERE  id_rel=$id AND lang='$l'";
        $mydbNW->ExecSql($sql);
    }
...
$sql = "SELECT ti.id , tt.* FROM guide as ti, guide_testi as tt  WHERE ti.id=$id AND ti.id=tt.id_rel";
$a_des = $mydbNW->DoSelect($sql);

if(count($a_des)==0)
{
    foreach($_vlanguage as $k => $v){
		    	$sql = "INSERT INTO guide_testi (id_rel, lang) VALUES ($id, '$v')";
                        $mydbNW->ExecSql($sql);
                        
    	}
        
    $sql = "SELECT t.id , tt.* FROM guide as ta, guide_testi as tt  WHERE ti.id=$id AND ti.id=tt.id_rel";
    $a_des = $mydbNW->DoSelect($sql);
}
